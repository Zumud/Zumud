...
from backend.utils.file_ops import PDFGenerator, extract_text_from_pdf, save_pdf
...
async def improve_resume_pdf(file: UploadFile = File(...)):
    ...
    pdf_file_path = save_pdf(
        str(save_path), latex_compiler_response.content, anonymous_username
    )
...
def edit_answer_with_instructions(
    ...
    try:
        ...
        try:
            ...
            session_info = get_current_session_info(current_user.username)
            company_name = (
                extract_company_from_local_path(save_path) or "unknown_company"
            )
...
def get_user_templates(
    ...
    templates = (
        ...
        .filter(db_models.UserTemplate.user_id == current_user.id)
        .order_by(db_models.UserTemplate.created_at.desc())
        .all()
    )
