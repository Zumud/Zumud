...
class StorageService:
    ...
    def _initialize_client(self) -> None:
        ...
        try:
            ...
            if self.supabase and SUPABASE_URL:
                ...
                logger.warning(
                    "Supabase client not available - storage service will be disabled"
                )
    ...
    def _ensure_bucket_exists(self) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Bucket {self.BUCKET_NAME} not found, attempting to create: {e}"
            )
        ...
        try:
            ...
            if response:
                ...
                logger.warning(
                    f"Failed to create bucket {self.BUCKET_NAME}: No response from Supabase"
                )
        except Exception as create_error:
            # If that fails, try with options dict
            try:
                logger.debug(
                    f"First attempt failed, trying with options: {create_error}"
                )
                ...
                if response:
                    logger.info(
                        f"Successfully created bucket {self.BUCKET_NAME} (with options)"
                    )
                else:
                    logger.warning(
                        f"Failed to create bucket {self.BUCKET_NAME}: No response from Supabase (with options)"
                    )
            except Exception as final_error:
                logger.error(
                    f"Failed to create bucket {self.BUCKET_NAME} with both methods: {create_error} | {final_error}"
                )
    ...
    def upload_file(
        self, file_path: str, file_content: bytes, content_type: str = "application/pdf"
    ) -> bool:
        """
        ...
        try:
            ...
            try:
                logger.debug(
                    f"First upload attempt failed, trying alternative format: {e}"
                )
                response = self.supabase.storage.from_(self.BUCKET_NAME).upload(
                ...
                if response:
                    logger.debug(
                        f"Successfully uploaded file to {file_path} (alternative format)"
                    )
                    return True
                else:
                    logger.warning(
                        f"Upload response was empty for {file_path} (alternative format)"
                    )
                    return False

            except Exception as final_error:
                logger.error(
                    f"Failed to upload file to {file_path} with both methods: {e} | {final_error}"
                )
                return False

    def upload_original_resume(
        self, user_id: int, file_content: bytes, filename: Optional[str] = None
    ) -> bool:
        """
    ...
    def upload_tailored_resume(
        ...
        timestamp = self._get_timestamp()
        base_path = self._construct_user_path(
            user_id, "applications", session_id, company_name
        )

        # Upload PDF
        pdf_filename = (
            self._construct_filename("resume", company_name, timestamp) + ".pdf"
        )
        ...
        if tex_content:
            tex_filename = (
                self._construct_filename("resume", company_name, timestamp) + ".tex"
            )
            tex_path = f"{base_path}/{tex_filename}"
            results["tex"] = self.upload_file(
                tex_path, tex_content.encode("utf-8"), "application/x-tex"
            )

        # Upload JSON if provided
        if json_content:
            json_filename = (
                self._construct_filename("resume", company_name, timestamp) + ".json"
            )
            json_path = f"{base_path}/{json_filename}"
            results["json"] = self.upload_file(
                json_path, json_content.encode("utf-8"), "application/json"
            )
    ...
    def upload_cover_letter(
        ...
        # Upload text file
        text_filename = (
            self._construct_filename("cover_letter", company_name, timestamp) + ".txt"
        )
        text_path = f"{base_path}/{text_filename}"
        results["text"] = self.upload_file(
            text_path, text_content.encode("utf-8"), "text/plain"
        )

        # Upload PDF if provided
        if pdf_content:
            pdf_filename = (
                self._construct_filename("cover_letter", company_name, timestamp)
                + ".pdf"
            )
            pdf_path = f"{base_path}/{pdf_filename}"
            results["pdf"] = self.upload_file(pdf_path, pdf_content, "application/pdf")
