...
- **One purpose per PR; the gates are strict.** CI enforces ruff (+format), import-linter layering (`api -> core -> utils -> models -> config`), knip, diff-coverage on changed lines, secret/dependency scans, an AI scope-coherence gate judged against `.cursor/rules/principles.mdc`, and the full test lanes. Touching a sensitive path (dependencies incl. `frontend/package-lock.json`, auth, billing, migrations, `.github/`, `deploy/`, `scripts/`) additionally needs maintainer sign-off: the "T2 approval" check reads your diff against the paths in `.github/labeler.yml`, and the `tier:T2` label it also gets is only for visibility. See `docs/dev-pipeline.md`.
